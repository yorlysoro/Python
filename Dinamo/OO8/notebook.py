...
class Notebook:
	...
	def modify_tags(self,note_id, tags):
		''''Find the note with the give id and change its tags to the given value'''
		for note in self.notes:
			if note.id  == note_id:
				note.tags = tags
				break
	def search(self, filter):
		'''Find all notes that match the fiven filter string'''
	...
	def __find_note(self, note_id):
		...
		return None
	def modify_memo(self, note_id, memo):
		'''Find the note with the given id and chan its memo to the given value'''
